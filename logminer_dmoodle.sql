...
BEGIN
   DBMS_LOGMNR_D.build (
   dictionary_filename => 'dictdmoodle.ora',  dictionary_location => '/u01/dict');
END;
...
BEGIN
   ...
   logfilename => '+DATA/dmoodle/onlinelog/group_1.279.889256575');

   DBMS_LOGMNR.add_logfile (
   options => DBMS_LOGMNR.addfile,
   logfilename => '+DATA/dmoodle/onlinelog/group_2.280.889256575');

   DBMS_LOGMNR.add_logfile (
   options => DBMS_LOGMNR.addfile,
   logfilename => '+DATA/dmoodle/onlinelog/group_3.281.889256575');
...
EXECUTE DBMS_LOGMNR.START_LOGMNR( -
   DICTFILENAME => '/u01/dict/dictdmoodle.ora', -
   STARTTIME => TO_DATE('03-Nov-2015 18:30:00', 'DD-MON-YYYY HH:MI:SS'), -
   ENDTIME => TO_DATE('03-Nov-2015 18:45:00', 'DD-MON-YYYY HH:MI:SS')); 
...
TRUNCATE TABLE t_logmnr_contents_UNIFESO

INSERT INTO t_logmnr_contents_UNIFESO as 
SELECT TIMESTAMP,OPERATION,TABLE_NAME, SQL_REDO
       FROM v$logmnr_contents where operation NOT IN ('INTERNAL', 'COMMIT', 'UNSUPPORTED', 'START') AND TABLE_space = 'RM_DADOS' 
       AND TO_char(TIMESTAMP,'hh24:mi') between '14:11' AND '14:27' order by timestamp;
